...
export function GodModeDashboard({
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-purple-600 hover:bg-purple-500 rounded-full flex items-center justify-center shadow-lg shadow-purple-900/50 transition-all z-50"
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 bg-gray-950 border border-purple-500/50 rounded-2xl shadow-2xl shadow-purple-900/30 z-50 overflow-hidden">
      ...
      <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
        ...
        <div>
          <button
            onClick={handleTick}
            disabled={isTicking}
            className="w-full py-2 px-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-900 disabled:cursor-not-allowed text-white font-semibold text-sm rounded-xl transition-colors"
          >
        ...
        <div>
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {bots.slice(0, 20).map(bot => (
              <div
                key={bot.id}
                className="flex items-center justify-between py-1.5 px-2 bg-white/5 rounded-lg hover:bg-white/10"
              >
                <div>
                  <div className="text-white text-xs font-medium">{bot.displayName}</div>
                  <div className="text-gray-500 text-xs">{bot.tier}</div>
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={() => onAscend(bot.id)}
                    className="px-2 py-0.5 text-xs bg-yellow-600/30 hover:bg-yellow-600/50 text-yellow-400 rounded-md transition-colors"
                    title="Ascend"
                  >
                    ▲
                  </button>
                  <button
                    ...
                    className="px-2 py-0.5 text-xs bg-red-600/30 hover:bg-red-600/50 text-red-400 rounded-md transition-colors"
                    title="Ban"
                  >
